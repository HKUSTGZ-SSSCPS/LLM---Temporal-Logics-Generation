...
def extract_location_info(response_text):
    ...
    location_info_pattern = re.compile(
        r"Location information:\s*{\s*Current Location:\s*{[\s\S]*?}\s*Target Location:\s*{[\s\S]*?}\s*Additional Details:\s*{[\s\S]*?}\s*}|"
        r"\{Location information:\s*Current Location:\s*{[\s\S]*?}\s*Target Location:\s*{[\s\S]*?}\s*Additional Details:\s*{[\s\S]*?}\s*\}",
        re.DOTALL
    )
...
def generate_and_print_ltl():
    ltl_prompt = (
        ...
        1.Navigation Instructions Requirements: Instructions should include multiple steps, such as turning left, turning right, going straight, etc. Each step should include specific distances or location descriptions. The final instruction should include arriving at the destination. 
    ...
    prompt_2 = f"""
        ...
        Please output without adding any text display symbols, such as (*#), only pure LTL expression text is needed.
        """
